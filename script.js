...
// Button Listener (Make sure you added the button in HTML!)
...
class Particle {
    constructor() {
        ...
        // **UPDATED**: Grab the current color from CSS variable instead of hardcoding
    ...
    for (let a = 0; a < particlesArray.length; a++) {
        for (let b = a; b < particlesArray.length; b++) {
            ...
            if (distance < (canvas.width/7) * (canvas.height/7)) {
                ...
                // Use the particle's own color for lines
                ctx.strokeStyle = particlesArray[a].color; 
                ctx.globalAlpha = opacityValue; // Handle opacity via globalAlpha
                ctx.lineWidth = 1;
                ctx.beginPath();
...
function typeWriter() {
    ...
    if (!heading) return; // Guard clause in case element is missing
